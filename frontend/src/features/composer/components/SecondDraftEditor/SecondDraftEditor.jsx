...
import {
  Sparkles, FileText, Linkedin, BookOpen, Lightbulb,
  RotateCcw, Copy, Check, Loader2, ChevronRight,
  Minus, Plus, Wand2
} from 'lucide-react';
  ...
  useEffect(() => {
    if (preFetchMutation.isSuccess) {
      ...
      if (data.sources.length === 0) {
        alert('No relevant memories found for this prompt. Try a different topic!');
      } else {
        setStep(2);
          ...
          return (
            <React.Fragment key={label}>
              <button
                ...
                disabled={!isDone}
              >
                <span className="sde-step-num">
                  {isDone ? '✓' : num}
                </span>
                <span className="sde-step-label">{label}</span>
              </button>
              {i < STEPS.length - 1 && (
                <span className={`sde-step-divider ${isDone ? 'is-done' : ''}`} />
              )}
            </React.Fragment>
          );
        })}
        ...
        {step === 2 && !isGenerating && (
          <div className="sde-guide fade-in">
            <div className="sde-guide-content">
              <p><strong>Memories Analyzed!</strong></p>
              <p>Select relevant memories from the sidebar, then click <strong>Generate</strong> below.</p>
            </div>
          </div>
        )}
      ...
      <div className="sde-action-bar">
        <div className="sde-actions-left">
          <button
            ...
            onClick={() => handleRefine('Expand this with more detail and examples.')}
            disabled={!content || isGenerating}
          >
            {isGenerating && refineText === '' ? <Loader2 size={11} className="sde-spin" /> : <Plus size={11} />}
            Expand
          </button>
          <button
            className="sde-action-btn"
            ...
            disabled={!content || isGenerating}
          >
            {isGenerating && refineText === '' ? <Loader2 size={11} className="sde-spin" /> : <Minus size={11} />}
            Shorten
          </button>
          <button
            className="sde-action-btn"
            onClick={() => handleRefine('Improve the structure and clarity of this draft.')}
            disabled={!content || isGenerating}
          >
            {isGenerating && refineText === '' ? <Loader2 size={11} className="sde-spin" /> : <Wand2 size={11} />}
            Improve
          </button>
        </div>
        ...
        <button 
          ...
          onClick={handleSave}
        >
          {isGenerating ? <Loader2 size={12} className="sde-spin" /> : <Check size={12} />}
          Save to Galaxy
        </button>
      </div>
